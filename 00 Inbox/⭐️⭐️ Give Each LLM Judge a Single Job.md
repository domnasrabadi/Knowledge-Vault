...
- Compared with a single judge, a collection of atomic and binary judges lets you:
    ...
    - If you use evals as a guardrail and the output needs to pass, you can save time and money. When one check fails, you can regenerate the output without waiting for the rest. With a combined judge, you have to wait for the full eval.
- Say we are using an eval as a quality gate that an agent needs to pass. Let's assume we have different criteria. With atomic checks, each check is a separate LLM judge. With a combined judge, all checks are part of a single LLM judge. Let's also assume each check has the same failure chance, which you can control in the simulation.
- The takeaway is that the higher the failure chance for each check, the faster the expected number of rechecks increases. The effect becomes more pronounced as the number of criteria increases: with 10 criteria at a 30% failure rate, the combined judge needs 9,718 rechecks, while the atomic checks need 3,000.
