...
- • **Make them argue** · two blind reviewers beat one confident guess • **Force divergence** · escape the cluster, don't chase it with bans • **Route like a frontier model** · one command loads one expert • **Give them memory** · runs that compound instead of restarting • **Scripts that talk back** · stdout is the instruction • **Hooks that fight back** · guardrails that fire uninvited • **Live-wire the browser** · the browser becomes a second input • **Compile to every harness** · one source, every runtime, every model • **Design for the weakest model** · gates that can't be skipped
...
- In essence, think of your [SKILL.md](http://SKILL.md) as an intelligent router, similar to a MoE (Mixture-of-Experts) LLM architecture. Your [SKILL.md](http://SKILL.md) could:
    - Route to specific sub-commands as inferred (for example, load [polish.md](http://polish.md) when the user requests something like "polish my page")
    - Load specific files only if a condition applies (auto-load [codex.md](http://codex.md) when the skill is used in Codex, or load [brand.md](http://brand.md) if the requested design is a landing page)
- Impeccable routes 23 commands through one [SKILL.md](http://SKILL.md) this way; each loads exactly one expert reference and leaves the other 22 out of context. You can too.
...
- But if you run into cross-harness/model issues, here's how I deal with it:
    ...
    - One source ([SKILL.src.md](http://SKILL.src.md)): "ask the user" compiles to AskUserQuestion on Claude / the input tool on Codex / plain chat on Cursor; the command prefix flips / to $; per-model bans compile only into that model's build (hardcode any of it and the skill degrades everywhere else)
