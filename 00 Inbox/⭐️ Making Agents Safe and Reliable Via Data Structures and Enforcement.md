...
- 1. Control logic in the prompt 2. Workflows that collapse into form-filling 3. Identity exposed as an agent input 4. Consumer modes as agent switches 5. Opaque tool failures with nowhere to go 6. Idempotency in the description, not the tool 7. Prompt constraints that are not measured
- Make the tool enforce it: @dataclass class WorkflowState: verification_passed: bool target_resolved: bool attempts: int agent = Agent("model", deps_type=WorkflowState) @agent.tool def perform_sensitive_action(ctx: RunContext[WorkflowState]) -> str: if not ctx.deps.verification_passed: return "Verification has not been completed." return _do_sensitive_action()
