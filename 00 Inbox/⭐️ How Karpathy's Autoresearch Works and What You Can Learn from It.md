...
- • Edit the training code • Run an experiment for a fixed amount of time • Measure the result using a fixed metric • Keep the change if it improves the score • Revert it if it does not • Repeat
...
- The second lesson is that **prompts are part of the architecture**. In Autoresearch, `[program.md](http://program.md)` is not fluff around the code. It defines workflow, boundaries, persistence, logging, recovery, and selection criteria
...
- The fourth lesson is that **time-bounded evaluation is underrated**. The 5-minute wall-clock budget is one of the best ideas in the repo
- The fifth lesson is that **reversibility and observability are non-negotiable**. Autoresearch keeps losers cheap to discard and makes every experiment inspectable through logs, commit history, and `results.tsv`.
