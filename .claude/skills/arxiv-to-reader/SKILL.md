...
Neither is a content loss, but both are real and unfixed — don't rediscover them from scratch.

- **Figures are emitted as `<embed>`, not `<img>`.** Pandoc uses `<embed src="…">` for graphics whose LaTeX reference has no raster extension; the script rewrites the `src` to a data URI but leaves the tag. The images *are* present in Reader's stored copy (verified by counting `data:image` occurrences, not `<img>` tags — counting `<img>` will wrongly report zero). Whether Reader actually renders `<embed>` is unconfirmed; if figures don't display, rewriting these to `<img>` is the one-line fix.
