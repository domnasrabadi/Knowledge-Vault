...
- **Two-part academic titles** (papers, mostly) follow the grammar `[coined term or hook] : [descriptive explanation]`. Cut or compress one half and keep whichever half the paper is actually recognizable by. This is a structural trigger — a colon or question mark splitting the title — with a subjective call inside it, since which half survives varies per paper and isn't predictable from the title alone:
    - `Results-Actionability Gap: Understanding How Practitioners Evaluate LLM Products in the Wild` → `How Practitioners Evaluate LLM Products in the Wild` (dropped the coined term)
    - `Spec-Driven Development: From Code to Contract in the Age of AI Coding Assistants` → `Spec-Driven Development (In the Age of AI Assistants)` (kept the term, compressed the subtitle to a parenthetical)
    - `Pairwise or Pointwise? Evaluating Feedback Protocols for Bias in LLM-Based Evaluation` → `Pairwise or Pointwise - Bias in LLM-Based Evaluation` (kept the hook, dropped the methodology clause)
- When a retitle is a **judgment call** — the two-part cut above, or a rewrite to match content — propose **three** alternatives and say which you'd pick, rather than a single option. Pure length trims, Title Case fixes, and character replacements need only one proposal.
