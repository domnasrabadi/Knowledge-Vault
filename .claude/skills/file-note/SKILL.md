...
2. **Read the note** and propose, in one compact summary for the user:
   ...
   - `topics`: 1–5 tags, ONLY from `40 System/Topic Taxonomy.md` — read that file and validate every tag against it. Never invent a tag; if none fits, propose leaving topics sparse or invoke the add-tag skill.
   - `quality`: from Reader ⭐️ tags or existing filename stars (1–3); empty if unrated
...
- Keep titles concise: shorten verbose source titles while keeping them recognizable.
