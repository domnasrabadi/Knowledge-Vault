...
status: raw
quality: 
topics: [agent-harnesses, ai-agents, context-engineering, ai-engineering]
...
- **Memory and search: continual learning**
    - The filesystem is again the primitive. Harnesses support memory file standards like `AGENTS.md` that get injected on every start.
        - As the agent edits that file, the harness reloads it, and knowledge from one session carries into the next. This is a crude but effective form of continual learning.
...
- **Battling context rot**
    ...
    - Three techniques show up repeatedly:
        1. **Compaction.** When the window gets close to full, something has to give.
        ...
        3. **Skills with progressive disclosure.** Loading every tool and MCP into context at startup degrades performance before the agent takes a single action.
