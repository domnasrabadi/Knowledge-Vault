...
status: raw
quality:
topics: [ai-tooling, multi-agent-systems]
...
# Fixing Hallucinated Citations Using Agents
...
- Most academic AI workflows live as one-off prompts in private chats.
    - The pipeline from literature search to draft to peer review to citation check to disclosure is rebuilt every time.
    - Academic Research Skills packages that pipeline as four Claude Code skills with mandatory human checkpoints at every stage.
